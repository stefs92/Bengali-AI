...
We also would like to find an effective way to normalize the data. We initially tried dividing the data by its max image size (255), however the program crashes when doing so, perhaps since floats are more unstable than integers for our model to work with. We hope to find a way to train the normalized data without our model crashing.
